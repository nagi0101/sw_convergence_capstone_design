...
입력: 희소 픽셀 집합 (N개, N은 가변)
  ...
  - Value: 그레이스케일 값 [0, 255]

출력: 재구성된 프레임 (H × W × 1)

아키텍처:
  1. Pixel Embedding Layer
     - 각 픽셀을 (x, y, value) → 임베딩 벡터로 변환
     - Sinusoidal positional encoding (continuous)

  2. Sparse Transformer Encoder (6 layers)
     - Self-attention on sparse pixels only
     - O(N²) complexity (N은 작으므로 매우 빠름)

  3. Cross-Attention Decoder (4 layers)
     - Query: 전체 프레임의 각 픽셀 위치 (H×W)
     - Key/Value: Sparse pixel embeddings (N)
     - 각 출력 픽셀이 인근 샘플 픽셀들을 참조

  4. CNN Refinement Head
     - 부드러운 재구성을 위한 후처리
     - 3-layer ConvNet (residual connection)
```
